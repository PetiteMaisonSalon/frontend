...
const FALLBACK_STAFF: Staff[] = [
  {
    ...
    firstName: "Mehtap",
    lastName: "",
    serviceIds: FALLBACK_SERVICES.map((s) => ({ _id: s._id })),
  },
  {
    _id: "fallback-sevim",
    firstName: "Sevim",
    lastName: "",
    serviceIds: FALLBACK_SERVICES.filter(
      (s) => s.category === "men" || s.name.toLowerCase().includes("färb"),
    ).map((s) => ({ _id: s._id })),
  },
  {
    _id: "fallback-maria",
    firstName: "Maria",
    lastName: "",
    serviceIds: FALLBACK_SERVICES.filter((s) => s.category === "women").map(
      (s) => ({ _id: s._id }),
    ),
  },
  {
...
export default function BuchungsFlow() {
  ...
  const [rescheduleToken, setRescheduleToken] = useState(
    searchParams.get("rescheduleToken") || "",
  );
  ...
  const [slotFilter, setSlotFilter] = useState<"all" | "morning" | "afternoon">(
    "all",
  );
  const [slots, setSlots] = useState<{ start: string; end: string }[]>([]);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState("");
  const [successType, setSuccessType] = useState<"booked" | "waitlist">(
    "booked",
  );
  ...
  const [monthDayHasSlots, setMonthDayHasSlots] = useState<
  ...
  useEffect(() => {
    const saved =
      typeof window !== "undefined"
        ? sessionStorage.getItem(BOOKING_STATE_KEY)
        : null;
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    getStaff()
      .then((data) => {
        setStaff(
          Array.isArray(data) && data.length > 0 ? data : FALLBACK_STAFF,
        );
      })
  ...
  useEffect(() => {
    ...
    if (parsedIds.length > 0) {
      matchedServices = services.filter((s) =>
        parsedIds.includes(String(s._id)),
      );
    ...
    try {
      if (isFallbackData) {
        ...
        const effectiveStaff =
          staffOverride === undefined ? selectedStaff : staffOverride;
        const data = await getAvailableSlots(
          date,
          selectedServiceIds,
          effectiveStaff?._id,
  ...
  const autoSelectAvailableStaff = async (date: string) => {
    ...
    const checks = await Promise.all(
      availableStaff.map(async (s) => {
        ...
        return {
          staff: s,
          hasSlots: Array.isArray(data) && data.length > 0,
          slots: data,
        };
      }),
    );
  ...
  const handleSubmit = async () => {
    if (selectedServices.length === 0 || !selectedStaff || !selectedSlot)
      return;
    ...
    if (isFallbackData) {
      setError(
        "Für echte Buchungen muss das Backend laufen. Starte das Backend (npm run dev) und führe den Seed aus (node src/scripts/seed.js).",
      );
      return;
    }
    ...
    try {
      ...
      setConfirmedBookingRef(
        aptId ? bookingRefFromMongoId(String(aptId)) : null,
      );
      goToStep(5);
    } catch (e: unknown) {
  ...
  if (step === 5) {
    ...
    const dateSerifLine =
      start &&
      `${start.toLocaleDateString("de-DE", { weekday: "short" })} ${start.toLocaleDateString(
        "de-DE",
        {
          day: "numeric",
          month: "long",
    ...
    const headlineSerif =
      dateSerifLine || (!isBooked ? "Warteliste bestätigt" : "");
        ...
        <div className="relative mx-auto max-w-lg">
          ...
          <div className="relative overflow-hidden rounded-[28px] bg-[#F2F0EB] px-6 pb-8 pt-12 shadow-[0_24px_80px_rgba(0,0,0,0.35)] sm:px-10 sm:pb-10 sm:pt-14">
            ...
            {isBooked && timeRangeLabel && (
              <p className="text-center text-[15px] font-medium tracking-tight text-[#1C1612]">
                {timeRangeLabel}
              </p>
            )}
            ...
            <div className="mt-8 rounded-2xl bg-white p-5 shadow-sm sm:p-6">
              ...
              <div className="divide-y divide-[#E8E8E6]">
                <div className="flex items-start justify-between gap-6 py-3 first:pt-0">
                  <span className="shrink-0 pt-0.5 text-[13px] text-[#1C1612]/55">
                    Name
                  </span>
                ...
                {isBooked ? (
                  <div className="flex items-end justify-between gap-4 border-t border-[#1C1612]/12 pt-5">
                    <span className="text-[15px] font-medium text-[#1C1612]">
                      Vor Ort bezahlen
                    </span>
                  ...
                  <p className="border-t border-[#1C1612]/12 pt-5 text-center text-[13px] text-[#1C1612]/55">
                    Sobald wir dir einen Termin anbieten können, gilt der
                    reguläre Preis – keine Zahlung über die Warteliste.
                  </p>
                )}
              </div>
            </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    {selectedServices.map((service) => (
                      ...
                      >
                        ...
                        <span aria-hidden className="text-[#1C1612]/70">
                          ×
                        </span>
                      </button>
                    ))}
                    ...
                    {availableStaff.map((s) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-xs text-[#1C1612]">
                            {s.firstName}
                          </span>
                        </button>
                      );
              ...
              <div>
                ...
                <div className="mt-4">
                  <div className="mb-3 flex items-center justify-between">
                    ...
                    <p className="text-xl text-[#1C1612] capitalize">
                      {monthLabel}
                    </p>
                    <button
                      type="button"
                  ...
                  <div className="grid grid-cols-7 gap-y-2 text-center text-xs text-[#1C1612]/75">
                    ...
                    {calendarDays.map((day, index) => {
                      ...
                      return (
                        <div
                          key={value}
                          className="mx-auto flex h-11 w-9 flex-col items-center justify-center"
                        >
                          <button
                            type="button"
                            disabled={isPast}
                          ...
                          {showAvailabilityDot && (
                            <span
                              className={`mt-1 h-1.5 w-1.5 rounded-full ${markerClass}`}
                            />
                          )}
                        </div>
                      );
            ...
            {!authLoading && !user && (
              <div className="px-5 pb-7 md:px-7">
                <div className="rounded-xl border border-[#CFC8BD] bg-[#ECE8E2] p-6 text-center">
                  <h3 className="text-h3 text-[#1C1612]">
                    Anmeldung erforderlich
                  </h3>
                  <p className="mt-2 text-[#1C1612]/85">
                    Du kannst nur mit einem bestätigten Konto einen Termin
                    verbindlich buchen.
                  </p>
                ...
                <div className="space-y-8">
                  <div>
                    ...
                    <div className="mt-4 grid gap-5 sm:grid-cols-2">
                      <div>
                        <label className="text-sm text-[#1C1612]/85">
                          Name
                        </label>
                        <input
                          type="text"
                  ...
                  <div>
                    ...
                    <p className="mt-4 text-[#1C1612]">
                      Wenn sich deine Pläne ändern, kannst du deinen Termin bis
                      24 Stunden vorher kostenlos umbuchen oder stornieren.
                    </p>
                    <p className="mt-4 text-[#1C1612]/85">
                      Bitte gib uns möglichst früh Bescheid, damit wir den
                      Termin neu vergeben können. Bei kurzfristigen Absagen oder
                      Nichterscheinen behalten wir uns vor, den Termin in
                      Rechnung zu stellen.
                    </p>
                  </div>
                </div>
                ...
                <aside className="space-y-4">
                  ...
                  <div className="overflow-hidden rounded-xl border border-[#DBD4CB] bg-[#F7F5F1]">
                    <div className="flex items-center justify-between border-b border-[#DDD7CE] px-4 py-3 text-sm text-[#1C1612]">
                      <span>Stylistin</span>
                      <span>
                        {selectedStaff ? selectedStaff.firstName : "Beliebig"}
                      </span>
                    </div>
                    <div className="flex items-center justify-between border-b border-[#DDD7CE] px-4 py-3 text-sm text-[#1C1612]">
                      <span>Datum</span>
                    ...
                    {selectedServices.map((s) => (
                      <div
                        key={s._id}
                        className="flex items-center justify-between border-b border-[#DDD7CE] px-4 py-3 text-sm text-[#1C1612]"
                      >
